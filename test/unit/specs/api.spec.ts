import VueRouter from '../../../src/entries/cjs'
...
describe('route matching', () => {
  ...
  it('can override currentRoute', () => {
    ...
    const { route, resolved } = router.resolve(
      { name: 'b' },
      {
        params: { id: '2' },
        path: '/a/2',
...
describe('router.addRoutes', () => {
  it('should work', () => {
    const router = new VueRouter({
      mode: 'abstract',
      routes: [{ path: '/a', component: { name: 'A' } }],
    })
    ...
    router.addRoutes([{ path: '/b', component: { name: 'B' } }])
    components = router.getMatchedComponents()
    expect(components.length).toBe(1)
...
describe('router.push/replace', () => {
  ...
  let router: VueRouter,
    spy1: jasmine.Spy<jasmine.Func>,
    spy2: jasmine.Spy<jasmine.Func>
...
describe('router app destroy handling', () => {
  ...
  let router: VueRouter, app1: Vue, app2: Vue, app3: Vue

  beforeEach(() => {
    router = new VueRouter({
      mode: 'abstract',
      routes: [{ path: '/', component: { name: 'A' } }],
    })
