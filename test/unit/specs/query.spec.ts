...
describe('Query utils', () => {
  describe('resolveQuery', () => {
    ...
    it('should keep null query values', () => {
      const query = resolveQuery('', { a: undefined, b: null } as any)
      expect(query).toEqual({ a: undefined, b: null } as any)
    })
    ...
    it('should keep objects query values', () => {
      ...
      expect(query).toEqual({ a: { nested: 'o' }, b: [{ a: true }] } as any)
    })
    ...
    it('should cast query values into string', () => {
      const query = resolveQuery('foo=bar&foo=k', { baz: 1 } as any)
      expect(query.baz).toBe('1')
    })

    it('should cast query array values into string', () => {
      const query = resolveQuery('foo=bar&foo=k', { baz: [1, '2'] } as any)
