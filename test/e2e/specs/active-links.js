...
module.exports = {
  ...
  "active links": function (browser) {
    ...
    function assertActiveLinks(n, activeA, activeLI, exactActiveA, exactActiveLI) {
      ...
      activeA.forEach((i) => {
        browser.assert.hasClass(`li:nth-child(${i}) a`, "router-link-active");
      });
      activeLI &&
        activeLI.forEach((i) => {
          browser.assert.hasClass(`li:nth-child(${i})`, "router-link-active");
        });
      exactActiveA.forEach((i) => {
        browser.assert
          .hasClass(`li:nth-child(${i}) a`, "router-link-exact-active")
          .assert.hasClass(`li:nth-child(${i}) a`, "router-link-active")
          .assert.attributeEquals(`li:nth-child(${i}) a`, "aria-current", "page");
      });
      exactActiveLI &&
        exactActiveLI.forEach((i) => {
          browser.assert
            .hasClass(`li:nth-child(${i})`, "router-link-exact-active")
            .assert.hasClass(`li:nth-child(${i})`, "router-link-active")
            .assert.attributeEquals(`li:nth-child(${i}) a`, "aria-current", "page");
        });
    }
  },
