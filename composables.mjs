...
function useRoute() {
    ...
    var thisRoot = root;
    if (!thisRoot._$route) {
        var route = effectScope(true).run(function () { return shallowReactive(Object.assign({}, thisRoot.$router.currentRoute)); });
...
function useLink(ref) {
    ...
    var resolvedRoute = computed(function () { return router.resolve(unref(to), currentRoute); });
    ...
    var isActive = computed(function () { return activeRecordIndex.value > -1 &&
        includesParams(currentRoute.params, resolvedRoute.value.route.params); });
    var isExactActive = computed(function () { return activeRecordIndex.value > -1 &&
